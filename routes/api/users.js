...
router.post("/", [
    ...
    check("email", "Nemáš oprávnění k registraci.").custom((value, {req})=>(value.includes("@creativehill.cz") && value.split("@")[0].includes("."))),
    ...
    try {
        ...
        if (user) {
            return res.status(400).json({ errors: [{ msg: "Uživatel s tímto emailem již existuje" }] })
        }
        user = new User({
                firstName,
                lastName,
        ...
        if(!yearOfEntry.includes("/")){
            if(isNaN(yearOfEntry.split("/")[0]) || isNaN(yearOfEntry.split("/")[1])){
...
router.get("/:token", async(req, res) => {
    ...
    if (!token) {
        return res.status(401).json({ msg: 'Nebyl nalezen token, ověření zamítnuto' });
    }
